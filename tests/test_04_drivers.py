...
def test_get_driver(client, mock_db_cursor):
    ...
    response = client.get("/api/v1/drivers/test-driver-id", headers=HEADERS)
    assert response.status_code == 200
...
def test_update_driver(client, mock_db_cursor):
    ...
    response = client.put("/api/v1/drivers/test-driver-id", json={
        "name": "Updated Driver"
    }, headers=HEADERS)
...
def test_delete_driver(client, mock_db_cursor):
    mock_db_cursor.rowcount = 1
    response = client.delete("/api/v1/drivers/test-driver-id", headers=HEADERS)
    assert response.status_code == 200
