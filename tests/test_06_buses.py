...
def test_delete_bus(client: TestClient, mock_db_cursor):
    # cascade_service.delete_cascades("buses", ...) calls execute_query(fetch_all=True)
    # to check for active trips. fetchall must return [] (no trips) for the guard to pass.
    # conftest now defaults fetchall to [] so no extra setup needed here.
    mock_db_cursor.rowcount = 1
    response = client.delete("/api/v1/buses/bus123", headers=HEADERS)
    assert response.status_code == 200
    data = response.json()
    assert data.get("message") == "Bus deleted successfully"
