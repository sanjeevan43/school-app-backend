...
def test_get_student(client, mock_db_cursor):
    ...
    response = client.get("/api/v1/students/test-student-id", headers=HEADERS)
    assert response.status_code == 200
...
def test_update_student(client, mock_db_cursor):
    ...
    response = client.put("/api/v1/students/test-student-id", json={
        "name": "Updated Student"
    }, headers=HEADERS)
...
def test_delete_student(client, mock_db_cursor):
    mock_db_cursor.rowcount = 1
    response = client.delete("/api/v1/students/test-student-id", headers=HEADERS)
    assert response.status_code == 200
