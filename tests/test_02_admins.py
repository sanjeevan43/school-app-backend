...
def test_create_admin(client, mock_db_cursor):
    ...
    response = client.post("/api/v1/api/v1/admins", json={
        "name": "Test Admin",
        "phone": 1234567890,
        "email": "admin@example.com"
    }, headers=HEADERS)
...
def test_get_all_admins(client, mock_db_cursor):
    ...
    response = client.get("/api/v1/api/v1/admins", headers=HEADERS)
    assert response.status_code == 200
...
def test_get_admin(client, mock_db_cursor):
    ...
    response = client.get("/api/v1/api/v1/admins/test-admin-id", headers=HEADERS)
    assert response.status_code == 200
...
def test_update_admin(client, mock_db_cursor):
    ...
    response = client.put("/api/v1/api/v1/admins/test-admin-id", json={
        "name": "Updated Admin"
    }, headers=HEADERS)
...
def test_delete_admin(client, mock_db_cursor):
    ...
    response = client.delete("/api/v1/api/v1/admins/test-admin-id", headers=HEADERS)
    assert response.status_code == 200
